...
> = Lazy::new(|| {
    Fallback::new(
        Batch::new(
            Verifier::new(/* Spend `VerifyingKey`*/),
        ...
        tower::service_fn(
            (|item: Item| {
                ready(item.verify_single(prepare_verifying_key(/* Spend `VerifyingKey`*/)))
            }) as fn(_) -> _,
        ),
    )
});
...
> = Lazy::new(|| {
    Fallback::new(
        Batch::new(
            Verifier::new(/* Output `VerifyingKey`*/),
            super::MAX_BATCH_SIZE,
            super::MAX_BATCH_LATENCY,
        ),
        // We want to fallback to individual verification if batch verification
        ...
        tower::service_fn(
            (|item: Item| {
                ready(item.verify_single(prepare_verifying_key(/* Output `VerifyingKey`*/)))
            }) as fn(_) -> _,
        ),
    )
});
...
> = Lazy::new(|| {
    Fallback::new(
        Batch::new(
            Verifier::new(/* JoinSplit `VerifyingKey`*/),
            super::MAX_BATCH_SIZE,
            super::MAX_BATCH_LATENCY,
        ),
        // We want to fallback to individual verification if batch verification
        ...
        tower::service_fn(
            (|item: Item| {
                ready(item.verify_single(prepare_verifying_key(/* JoinSplit `VerifyingKey`*/)))
            }) as fn(_) -> _,
        ),
    )
});
